...
- **§1 / user story 20 models (issue 18)**: the root agent (`AGENT_MODEL_ID`) and System 2 (`SYSTEM2_MODEL_ID`) are configured separately. Both still default to `anthropic/claude-sonnet-5`. The requested switch to `anthropic/claude-haiku-4.5` was tried and reverted. Through the Gateway, Haiku did not produce structured output: the root's turn outcome failed in 4 of 4 live sessions, and System 2 stopped after its first property in 5 of 5 calls. Haiku is still selectable and fails loudly. This needs a user decision.
