...
- **Jev request details (issue 07)**: `zeroDataRetention` is temporarily **off**, because the Gateway refuses it on the Pro Trial plan. Restore it once the team is on a paid Pro plan. Score questions use 3 band rungs, because Jev allows at most 10 levels; the fractional position is scaled to 0–10.
