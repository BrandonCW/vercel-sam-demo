...
- **§1 / user story 20 models (issue 18)**: the root agent runs on `anthropic/claude-haiku-4.5` (`AGENT_MODEL_ID`). System 2 keeps `anthropic/claude-sonnet-5` as its default (`SYSTEM2_MODEL_ID`). Haiku 4.5 was asked for, but through the Gateway it stops after the first property of the System 2 object and fails validation, every time. It stays selectable, and fails loudly if chosen. This needs a user decision.
