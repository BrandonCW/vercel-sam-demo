...
Implementation detail for people working on this repository. For an overview of what the project does, see the [README](../README.md).
...
All environments share one Neon project (replace `<neon-project-id>` below with its ID):

| Branch | Used by | Seed command |
|---|---|---|
| `main` | dev, preview and production | `POSTGRES_URL="$(npx neonctl connection-string main --project-id <neon-project-id>)" pnpm db:seed` |
...
- **One tool, sequenced in code.** Each Assessment Session is one durable eve session and one call to the `run_assessment` workflow tool (`agent/tools/run_assessment.ts`, `"use workflow"`). Its `"use step"` functions (`agent/lib/assessment-steps.ts`) score with Jev, run System 2, record the SA answers, re-score and write back. The root model only starts it (`assessTurnMessage`) and replies in one line.
- **Code decides pass or fail (issue 20).** The verdict is the `run_assessment` action itself. A completed action whose output parses as `AssessmentResultSchema` (`lib/assessment-progress.ts`: `status: "written_back"`, scores, delta, next steps, `writebackId`) is a pass. A failed action is a failure, with the tool's own error. No `outputSchema` is sent, and nothing the root model says is read. The workbench also reads the requested model and writeback flag from the action's input in the stream.
...
- **What the UI renders.** `lib/assessment-results.ts` projects the stream into the view. The tool's yields arrive as `action.partial` events: the Jev scores before their CRM write, then each persisted result. `action.result` carries the verdict. Failures show verbatim.
